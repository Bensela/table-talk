# Table-Talk MVP - Development Rules v1.2.1
...
3. **Join Codes**: 6-digit codes for secure Dual-Phone pairing (10-min expiry, modal auto-dismiss on join)
4. **Smart Reconnection**: Phone A (creator) auto-rejoins existing session on QR rescan
5. **Dual-Phone Slots**: 2 participants max, Phone C denied if A+B paired
6. **Deterministic Deck**: Seed-based shuffle per group+table+context+day, no repeats, wraps at end
7. **Dual-Phone Sync**: WebSocket <300ms latency, participant-based rooms, "Both Ready" + checkmark reveal rituals
...
sessions: session_id, table_token, restaurant_id, mode, context, session_group_id, 
          pairing_code_hash, pairing_expires_at, dual_status, paired_count, expires_at, last_activity_at
...
## Dual-Phone UX Rules (v1.2.1)
- **Code Display**: Modal dialog on Phone A, dismisses when Phone B joins
- **Pairing Limit**: Max 2 participants (A + B), Phone C denied if slots full
- **Creator Reconnect**: Phone A rescanning QR auto-rejoins if participant_id in sessionStorage
- **Join Flow**: Phone B gets code from Phone A screen, enters on own device
- **Third Party Block**: Phone C attempting join when paired gets "Session full" error
...
## Milestones
- Phase 1 (20h): Setup, QR, welcome, context/mode, session groups, join codes, modal UI
- Phase 2 (35h): Deck logic, questions, Single+Dual modes, participant sync, reconnect
...
- Join code: 6-digit numeric, SHA256 hashed with salt, rate-limited (5 attempts/min)
- Modal: Auto-dismiss on pair, closeable manually, responsive mobile design# Table-Talk MVP - Development Rules v1.2
